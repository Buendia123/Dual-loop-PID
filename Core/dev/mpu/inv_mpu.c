...
/* Delay is a function already defined in ASF. */
...
void DMP_Init(void)
{
	uint8_t temp;
	uint8_t offset = 0x75;
	IICreadBytes(0xD0,offset,1,&temp);
...
//	  run_self_test();
	  mpu_set_dmp_state(1);
	  OLED_ShowString(0,0,"DMP successfully",8,1);
	  OLED_Refresh();
	  Delay(100);
  }
	else{
		OLED_ShowString(0,0,"DMP failed",8,1);
		OLED_Refresh();
		Delay(100);
	}
